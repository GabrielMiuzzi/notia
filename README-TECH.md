...
Validaciones nativas registradas para esta área: `npm run build` y `cargo check --manifest-path src-tauri/Cargo.toml`. `cargo test` compiló pero el ejecutable de pruebas no pudo iniciarse en este entorno Windows por `STATUS_ENTRYPOINT_NOT_FOUND`; queda pendiente repetir las pruebas nativas en un entorno de ejecución compatible. Para este ajuste de interfaz también pasaron `npm run lint` y `npm test -- --run --maxWorkers=1 --no-file-parallelism` (110 archivos, 568 pruebas). Sigue pendiente la prueba manual de UI y la validación en dispositivo Android físico.
