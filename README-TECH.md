...
Todos los chats de la aplicación —vista principal, panel lateral y Telegram— usan el mismo runtime agente de `chatScopedAgentRuntime.ts`, el mismo catálogo completo de herramientas y `runNativeToolAgent`. El transporte envía schemas en `tools` a `/api/chat`, procesa `message.tool_calls`, valida y ejecuta cada llamada localmente, agrega resultados con rol `tool` y repite hasta obtener una respuesta final. Las diferencias de scope (`library`, `task-manager`, `graph`, `document`) solo determinan el contexto inicialmente autorizado y el tablero activo; no eliminan herramientas. Las escrituras se serializan, requieren confirmación individual y las solicitudes compuestas usan un plan aprobado antes de ejecutar. La capacidad informada por `/api/show` se presenta como ayuda en el selector, pero no bloquea preventivamente la ejecución porque algunos modelos de Ollama Cloud omiten esos metadatos; `/api/chat` es la autoridad final y devuelve un error si la variante rechaza herramientas.
...
`qwen3_asr_service` implementa el adapter del bridge `llama.cpp`. Los parciales se calculan cada 1,5 segundos únicamente cuando llegó voz nueva, sobre ventanas de 12 segundos con 2 segundos de solapamiento; el texto compartido se reconcilia al desplazar la ventana. Un margen de 1,1 segundos evita endpoints dentro de pausas breves y cada endpoint se confirma procesando la intervención completa. El adapter respeta la preferencia CPU/GPU y reutiliza el runtime mientras no cambie. En Windows, seleccionar GPU exige que el runtime incluya `notia_asr_ggml-vulkan.dll`; la carga falla de forma explícita si solo está instalado el runtime CPU, evitando presentar una aceleración inexistente. La captura mantiene una cola PCM acotada de 15 segundos para absorber inferencias transitoriamente más lentas que tiempo real sin cortar audio. `sherpa_diarization` usa segmentación pyannote, embeddings y clustering automático conservador (`threshold = 0.9`, sin fijar cantidad de hablantes), descarta activaciones menores a 500 ms y une pausas menores a 300 ms. Los handles son RAII. La diarización corre exclusivamente al finalizar la sesión; si falla, se conserva el ASR sin etiquetas.
