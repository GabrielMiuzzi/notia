...
La captura nativa usa `cpal`. El reconocimiento admite dos motores: Parakeet TDT 0.6B v3 int8 (por defecto) sobre la C API de sherpa-onnx 1.13.4, que también ejecuta Silero VAD y la diarización, y Qwen3-ASR con un bridge C sobre `llama.cpp`/`libmtmd`. El mismo servicio Rust opera en Windows y Android. Android agrega un plugin Kotlin mínimo que declara y solicita `RECORD_AUDIO` sólo desde la acción explícita del usuario.

Cada perfil de modelos se resuelve de forma independiente desde la primera carpeta que contiene todos sus archivos declarados: en debug, primero los recursos del checkout; después `app_data_dir/speech-models`, y por último los recursos empaquetados, contra los que se informan los archivos faltantes. Así, una instalación privada incompleta o de otro perfil no oculta un perfil empaquetado completo. El manifiesto declara el perfil `es-parakeet-tdt-v3` (`offlineNemoTransducer`: encoder, decoder, joiner, tokens y Silero VAD), Qwen3-ASR 0.6B/1.7B Q8 como pares de modelo y proyector multimodal GGUF, y los modelos independientes de diarización. `build.rs` falla si faltan los archivos de Parakeet o de Qwen3-ASR 0.6B; Parakeet no se versiona en Git y se instala con `bash scripts/install-speech.sh parakeet`, que verifica los SHA-256 del manifiesto. Android empaqueta sólo los cinco archivos del perfil Parakeet. Las rutas, tamaños y SHA-256 se verifican antes de cargar.

La preferencia de dispositivo `qwen3Asr` conserva su nombre persistido y acepta `model`: `parakeet-v3` (valor por defecto y de fallback ante valores desconocidos), `0.6b` o `1.7b`. `backend_core::device_preferences::normalize_asr` conserva un tamaño Qwen3-ASR ya guardado, por lo que una instalación previa sigue con Qwen3-ASR hasta que se elija Parakeet. `prepare_speech_model` y `start_speech_session` validan `model` con `speech_model_repository::is_supported_asr_model` y `device` en `cpu | gpu`; `resolve_asr_model` devuelve `AsrModelConfig::Parakeet` o `AsrModelConfig::Qwen3`. Parakeet ignora `device` (sólo CPU, hasta 4 threads) y detecta el idioma; Qwen3-ASR usa `device` y fuerza el idioma solicitado en su prefijo de decodificación.

`asr_recognizer::AsrRecognizer` envuelve ambos adapters e implementa `StreamingRecognizer`; el caché precargado, el worker, las notas de voz de Telegram y la segunda pasada por turnos de la diarización sólo usan ese tipo. `matches` compara la configuración resuelta completa para reutilizar el modelo residente o reemplazarlo.
...
Con Qwen3-ASR, la sesión vuelve a decodificar el tramo activo y publica el resultado como `partialText`. Un detector de energía acotado confirma el tramo después de 700 ms de silencio; la UI conserva el parcial resaltado hasta ese momento. La diarización continúa ejecutándose únicamente al finalizar la sesión, con ventanas acotadas cuando el audio supera 15 minutos y matching global de embeddings para conservar la identidad de los hablantes entre ventanas.

El idioma configurado se normaliza en frontend y se valida en Rust. Qwen3-ASR lo traduce al nombre esperado para que una sesión en español no cambie de idioma por autodetección inestable; Parakeet sólo lo usa para elegir la normalización del texto.
...
Inicio: `{"language":"es","model":"parakeet-v3","device":"cpu","diarizationEnabled":true,"maxDurationSeconds":900}`. Respuesta: `{"sessionId":"7a5dd258-2675-47cc-a32d-01ef4f414946"}`. Los controles reciben el mismo `sessionId`.
...
flowchart LR
    ...
    Worker --> Engine[AsrRecognizer]
    Engine --> Parakeet[sherpa-onnx: Parakeet TDT + Silero VAD]
    Engine --> Qwen[llama.cpp + Qwen3-ASR]
