...
#[serde(rename_all = "camelCase")]
pub(crate) struct ReindexPayload {
    library_id: String,
}

/// Rebuilds the inventory of a registered library.
#[tauri::command]
pub(crate) async fn backend_reindex_library(app: AppHandle, payload: ReindexPayload) -> ReindexResult {
    let result = tauri::async_runtime::spawn_blocking(move || reindex_library(&app, &payload.library_id))
        .await
        .unwrap_or_else(|_| {
            Err(BackendError::new(
                BackendErrorCode::Storage,
                "La reindexación se interrumpió.",
                true,
            ))
        });
    match result {
        Ok((indexed, generation)) => ReindexResult {
            ok: true,
            indexed,
            generation,
            error: None,
        },
        Err(error) => ReindexResult {
            ok: false,
            indexed: 0,
            generation: 0,
