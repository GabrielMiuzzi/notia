...
pub(crate) fn init() -> tauri::plugin::TauriPlugin<tauri::Wry> {
    tauri::plugin::Builder::new("library-registry")
        .setup(|app, _api| {
            #[cfg(not(target_os = "android"))]
            if let Ok(directory) = tauri::Manager::path(app).app_data_dir() {
                tauri::Manager::state::<LibraryBindingRegistry>(app)
                    .load_persisted(directory.join(BINDINGS_FILE));
            }
...
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct RegisterLibraryBindingPayload {
    pub(crate) library_id: String,
...
#[tauri::command]
pub(crate) fn register_library_binding(
    payload: RegisterLibraryBindingPayload,
    registry: tauri::State<'_, LibraryBindingRegistry>,
) -> Result<(), BackendError> {
    #[cfg(target_os = "android")]
    {
        let tree_uri = payload.android_tree_uri.as_deref().ok_or_else(|| {
            BackendError::invalid_input("La biblioteca Android necesita un grant SAF.")
        })?;
        registry.register_android_tree(&payload.library_id, tree_uri)?;
        return Ok(());
    }

    #[cfg(not(target_os = "android"))]
    {
        let _ = payload.android_tree_uri;
        registry.register_desktop_root_from_client(&payload.library_id, Path::new(&payload.library_path))?;
        Ok(())
    }
}
...
#[tauri::command]
pub(crate) fn pick_library_directory(
    _payload: PickLibraryDirectoryPayload,
) -> Result<Option<PickedLibraryDirectory>, BackendError> {
    Err(BackendError::new(
        BackendErrorCode::Unsupported,
        "El selector de bibliotecas de escritorio no está disponible en esta plataforma.",
        false,
    ))
}
...
pub(crate) fn revoke_library_binding(
    ...
    library_id: String,
    registry: tauri::State<'_, LibraryBindingRegistry>,
    backend: tauri::State<'_, crate::backend_runtime::BackendRuntimeState>,
    coldpass: tauri::State<'_, crate::coldpass::ColdPassState>,
) -> Result<(), BackendError> {
