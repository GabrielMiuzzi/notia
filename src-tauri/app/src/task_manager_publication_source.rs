...
/// Tells the publication of the library that the host changed its Task
/// Manager, with the published boards' settings rebuilt from the store.
pub(crate) fn announce_store_change(app: &AppHandle, library_id: &str) {
    let publication = app.state::<crate::task_manager_publication::TaskManagerPublicationState>();
    if !crate::task_manager_publication::is_publishing_library(&publication, library_id) {
        return;
    }
    let settings = build_payload(app, library_id, "dark")
        .ok()
        .and_then(|payload| payload.get("settings").cloned());
    crate::task_manager_publication::announce_host_change(&publication, library_id, settings);
}
...
pub(crate) async fn backend_publish_task_manager(app: AppHandle, payload: PublishPayload) -> Result<String, String> {
    crate::host::async_runtime::spawn_blocking(move || publish(&app, &payload.library_id, payload.theme.as_deref().unwrap_or("dark")))
        .await
...
pub(crate) fn init() -> crate::host::plugin::TauriPlugin<crate::host::Wry> {
    crate::host::plugin::Builder::new("task-manager-publication-autostart")
        .setup(|app, _api| {
            #[cfg(target_os = "windows")]
            {
                let app = app.clone();
