...
impl Qwen3AsrRecognizer {
    ...
    /// Qwen3-ASR runs on the llama.cpp runtime built for Windows and Android;
    /// other platforms (the Linux server) have no such runtime.
    #[cfg(not(any(target_os = "windows", target_os = "android")))]
    pub fn load(_app: &AppHandle, _config: &Qwen3AsrModelConfig) -> Result<Self, String> {
        Err("Qwen3-ASR no está disponible en esta plataforma.".to_string())
    }
...
fn runtime_directory(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    #[cfg(debug_assertions)]
    return Ok(PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/.."))
        .join("resources")
        .join("qwen3-asr")
        .join("runtime")
        .join(platform));
...
mod tests {
    use super::{
        ...
        next_partial_interval_samples, CPU_ENDPOINT_OVERLAP_SAMPLES, CPU_MAX_UTTERANCE_SAMPLES,
