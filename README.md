...
- Los chats incluyen un **modo charla** mediante el botón de llamada junto al micrófono. Qwen3-TTS 0.6B CustomVoice Q4_K_M reproduce el saludo y cada respuesta; Sherpa-ONNX mantiene una sesión de escucha continua, separa los turnos mediante VAD y los envía al detectar la pausa configurada. La captura solo se pausa mientras habla la IA y se reanuda sin recrear el micrófono. Ambos motores se precargan al abrir Notia y funcionan localmente, sin servidor de voz ni Python, en Windows y Android arm64. La sección **Configuraciones → Voz** permite activar el modo y elegir una de las nueve voces de Qwen, idioma, velocidad, pausa y saludo. Los pesos se instalan y verifican con `scripts/install-qwen3-tts-models.ps1`; el runtime GGML nativo se construye mediante `scripts/build-qwen3-tts-runtime.ps1`.
