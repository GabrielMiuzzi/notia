...
if ($Device -eq 'gpu') { $arguments += '-DGGML_VULKAN=ON' }
...
$vulkanLibraryName = if ($Platform -eq 'windows-x86_64') { 'notia_asr_ggml-vulkan.dll' } else { 'libnotia_asr_ggml-vulkan.so' }
if ($Device -eq 'gpu') {
  $vulkanRuntime = Get-ChildItem -LiteralPath $build -Recurse -Filter $vulkanLibraryName |
    Select-Object -First 1
  if (-not $vulkanRuntime) {
    throw "El runtime GPU se compiló sin el backend Vulkan $vulkanLibraryName."
  }
}
