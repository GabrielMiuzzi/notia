...
| Meeting/voz | control de sesión, fuentes, prueba de audio y comandos `meeting_*` | usuario / meeting | document read-only | ephemeral-no-memory | estado de captura, parcial, niveles, progreso de diarización, `meeting://changed`, `meeting://answer` | snapshot de la reunión (turnos, hablantes, IA), nota Markdown y exportación |
