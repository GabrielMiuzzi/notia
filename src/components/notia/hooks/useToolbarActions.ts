...
import { setActiveTabPath, activateSpecialTab, setContextMenu, setRenamingPath, setPendingCreation, GRAPH_WORKSPACE_TAB_PATH, CHAT_WORKSPACE_TAB_PATH, TASK_MANAGER_WORKSPACE_TAB_PATH, COLDPASS_WORKSPACE_TAB_PATH, MEETING_WORKSPACE_TAB_PATH, FINANCE_WORKSPACE_TAB_PATH, CALENDAR_WORKSPACE_TAB_PATH, AGENDA_WORKSPACE_TAB_PATH, MULTICHAT_WORKSPACE_TAB_PATH, ROUTINE_WORKSPACE_TAB_PATH } from '../../../features/documents/documentsSlice'
...
export function useToolbarActions({
  ...
  const handleRailActionClick = useCallback((actionId: string) => {
    ...
    if (actionId === 'agenda') {
      if (!specialTabs.agenda) { dispatch(activateSpecialTab('agenda')) }
      dispatch(setActiveTabPath(AGENDA_WORKSPACE_TAB_PATH))
      return
    }
