...
import type { InkMathPoint, InkMathStroke } from "../types";
...
const lerpPoint = (a: InkMathPoint, b: InkMathPoint, t: number): InkMathPoint => ({
	x: lerp(a.x, b.x, t),
	y: lerp(a.y, b.y, t),
	pressure:
...
const getAveragePressure = (points: InkMathPoint[]): number => {
	let total = 0;
	let count = 0;
...
const getAverageTiltIntensity = (points: InkMathPoint[]): number => {
	let total = 0;
	let count = 0;
	for (const point of points) {
...
export const resolveStrokeRenderWidth = (stroke: InkMathStroke, brush: BrushPreset): number => {
...
export const resolveStrokeRenderOpacity = (stroke: InkMathStroke, brush: BrushPreset): number => {
...
const drawPath = (ctx: CanvasRenderingContext2D, points: InkMathPoint[]): void => {
...
const resolveStylusDynamics = (point: InkMathPoint, fallbackAzimuth: number): StylusDynamics => {
...
const getSegmentDistance = (a: InkMathPoint, b: InkMathPoint): number =>
	Math.sqrt((b.x - a.x) ** 2 + (b.y - a.y) ** 2);

const resolveVelocityNormalized = (prev: InkMathPoint, curr: InkMathPoint, baseWidth: number): number => {
...
const strokeHasStylusData = (points: InkMathPoint[]): boolean =>
	points.some(
		(point) =>
