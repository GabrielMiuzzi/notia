...
import { getIcon, getIconIds } from "../platform/inkmathPlatform";
...
const clearTextFallback = (element: HTMLElement): void => {
	const host = element.querySelector<HTMLElement>(".inkmath-icon-render-host");
	if (host) {
		host.remove();
	}
	const fallback = element.querySelector<HTMLElement>(".inkmath-icon-text-fallback");
	if (fallback) {
		fallback.remove();
	}
...
const setTextFallbackText = (element: HTMLElement, value: string): void => {
	...
	element.createSpan({ cls: "inkmath-icon-text-fallback", text: value });
};
...
const getRenderHost = (element: HTMLElement): HTMLElement => {
	...
	const existing = element.querySelector<HTMLElement>(".inkmath-icon-render-host");
	if (existing) {
		existing.empty();
		return existing;
	}
	return element.createSpan({ cls: "inkmath-icon-render-host" });
};
...
export const setInkMathIcon = (element: HTMLElement, name: string, textFallback = "?"): void => {
...
export const setInkMathMenuItemIcon = (item: MenuItemLike, name: string): void => {
	const known = getKnownIconIds();
	const candidates = resolveIconCandidates(name);
	for (const id of candidates) {
		if (known.has(id)) {
