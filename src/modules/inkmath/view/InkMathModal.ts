import { App, Modal, requestUrl } from "../platform/inkmathPlatform";
import {
...
import type { InkMathStroke as DocumentStroke } from "../types";
...
type InkMathStroke = DocumentStroke & { mode: "draw" };
...
export class InkMathModal extends Modal {
	...
	private readonly recognizeLatex: InkMathModalOptions["recognizeLatex"];
	...
	private lastStabilizedDrawPoint: InkMathPoint | null = null;
	...
	onOpen(): void {
		this.detachShell = attachInkMathModalEngine(this, { tone: "inkmath", size: "xl" });
		this.modalEl.addClass("inkmath-modal-shell");
		this.contentEl.addClass("inkmath-modal");
		this.createLatexPreview();
		this.setLatexInvalidAlert(false);
		void this.renderLatexPreview();

		this.canvasShellEl = this.contentEl.createDiv({ cls: "inkmath-canvas-shell" });
		this.canvasEl = this.canvasShellEl.createEl("canvas", { cls: "inkmath-canvas" });
		this.previewCanvasEl = this.canvasShellEl.createEl("canvas", { cls: "inkmath-canvas-preview" });
		...
		this.appendLog(this.recognizeLatex ? "InkMath listo. OCR: Ollama." : `InkMath listo. OCR: ${this.serviceUrl}/v1/inkmath/recognize`);
		this.appendLog(`Debounce OCR: ${this.ocrDebounceMs} ms.`);
		this.updateTelemetryUi();
		if (!this.recognizeLatex) {
	...
	onClose(): void {
		...
		this.modalEl.removeClass("inkmath-modal-shell");
		this.contentEl.empty();
	}

	private createLatexPreview(): void {
		const preview = this.contentEl.createDiv({ cls: "inkmath-latex-preview" });
		this.latexPreviewEl = preview;
		this.latexPreviewContentEl = preview.createDiv({ cls: "inkmath-latex-preview-content" });
		const editButton = preview.createEl("button", {
			cls: "inkmath-latex-edit-button",
			attr: {
				type: "button",
		...
		setInkMathIcon(editButton, "pencil", "E");
	...
	private setLatexInvalidAlert(visible: boolean): void {
		...
		if (!this.latexInvalidIconEl || this.latexInvalidIconEl.parentElement !== this.latexPreviewEl) {
			this.latexInvalidIconEl = this.latexPreviewEl.createSpan({
				cls: "inkmath-latex-invalid-icon"
			});
	...
	private onPointerDown = (event: PointerEvent): void => {
		...
		if (this.pointerMode === "erase") {
			...
			const stroke: InkMathStroke = {
				...
				opacity: 1,
				style: resolveInkMathStrokeStyle(brush.style),
				tool: "pen",
				brushId: brush.id,
	...
	private async sendRecognizeRequest(
		...
		if (this.recognizeLatex) {
			...
			const latex = await this.recognizeLatex(imageBase64, abortSignal);
			return {
				latex,
				time_ms: performance.now() - startedAt,
				cold_start: false,
				device: "ollama"
			};
		}
	...
	private createToolRow(): void {
		const toolRow = this.contentEl.createDiv({ cls: "inkmath-tools" });
		...
		const createToolButton = (
			...
			const button = toolRow.createEl("button", {
				cls: "inkmath-tool-button",
				attr: { type: "button", "aria-label": label, title: label }
			});
			const iconEl = button.createSpan({ cls: "inkmath-tool-icon" });
			setCompatibleIcon(iconEl, icon, fallback);
			button.createSpan({ cls: "inkmath-tool-label", text: label });
		...
		const strokeWidthControl = toolRow.createDiv({ cls: "inkmath-stroke-width" });
		strokeWidthControl.createDiv({ cls: "inkmath-stroke-width-label", text: "Grosor" });
		this.strokeWidthSliderEl = strokeWidthControl.createEl("input", {
			cls: "inkmath-stroke-width-slider",
			attr: {
				type: "range",
		...
		this.strokeWidthValueEl = strokeWidthControl.createDiv({
			cls: "inkmath-stroke-width-value",
	...
	private createToggleToolButton(
		...
		const button = toolRow.createEl("button", {
			cls: "inkmath-tool-button inkmath-toggle",
			attr: { type: "button", "aria-label": label, title: label }
		});
		const iconEl = button.createSpan({ cls: "inkmath-tool-icon" });
		setCompatibleIcon(iconEl, icon, fallback);
	...
	private createLogsCard(): void {
		const logsCard = this.contentEl.createDiv({ cls: "inkmath-logs-card" });
		const header = logsCard.createDiv({ cls: "inkmath-card-header" });
		header.createDiv({ cls: "inkmath-logs-title", text: "Logs" });
		const collapseButton = header.createEl("button", {
			cls: "inkmath-card-toggle",
		...
		this.logsBodyEl = logsCard.createDiv({ cls: "inkmath-logs-body" });
		this.updateCardCollapseUi();
	}

	private createTelemetryCard(): void {
		const telemetryCard = this.contentEl.createDiv({ cls: "inkmath-telemetry-card" });
		const cardHeader = telemetryCard.createDiv({ cls: "inkmath-card-header" });
		cardHeader.createDiv({ cls: "inkmath-logs-title", text: "Telemetría OCR" });
		...
		const header = telemetryCard.createDiv({ cls: "inkmath-telemetry-header" });
		this.telemetryStatusEl = header.createDiv({ cls: "inkmath-ocr-status is-idle", text: "Idle" });
		this.telemetryDebounceValueEl = header.createDiv({
			cls: "inkmath-telemetry-debounce-value",
			text: "500 ms"
		});

		const debounceTrack = telemetryCard.createDiv({ cls: "inkmath-telemetry-debounce-track" });
		this.telemetryDebounceBarFillEl = debounceTrack.createDiv({
			cls: "inkmath-telemetry-debounce-fill"
		});
		this.telemetryBackendMessageEl = telemetryCard.createDiv({
			cls: "inkmath-telemetry-backend",
			text: "Backend: verificando /health..."
		});

		this.telemetryTimelineEl = telemetryCard.createDiv({ cls: "inkmath-telemetry-timeline" });
		this.telemetryCardEl = telemetryCard;
		this.updateCardCollapseUi();
	}

	private createActionsRow(): void {
		const actions = this.contentEl.createDiv({ cls: "inkmath-actions" });
		this.actionsEl = actions;
		const debugButton = actions.createEl("button", {
			cls: "inkmath-action inkmath-debug-eye",
			attr: { type: "button", "aria-label": "Ver imagen OCR", title: "Ver imagen OCR (debug)" }
		});
		setCompatibleIcon(debugButton, "search", "O");
		const rightGroup = actions.createDiv({ cls: "inkmath-actions-right" });
	...
	private updateTelemetryUi(): void {
		...
		this.telemetryStatusEl.className = `inkmath-ocr-status is-${this.status}`;
		...
		const fraction =
			(this.ocrDebounceMs - INKMATH_OCR_DEBOUNCE_MIN_MS) /
			(INKMATH_OCR_DEBOUNCE_MAX_MS - INKMATH_OCR_DEBOUNCE_MIN_MS);
		const safeFraction = Math.max(0, Math.min(1, fraction));
		this.telemetryDebounceBarFillEl.style.width = `${(safeFraction * 100).toFixed(2)}%`;
		if (this.telemetryBackendMessageEl) {
			this.telemetryBackendMessageEl.className = `inkmath-telemetry-backend ${
	...
	private renderTimeline(): void {
		...
		for (const entry of latestFirst) {
			const row = this.telemetryTimelineEl.createDiv({ cls: "inkmath-telemetry-row" });
			row.createDiv({ cls: "inkmath-telemetry-row-id", text: `#${entry.requestId}` });

			const barWrap = row.createDiv({ cls: "inkmath-telemetry-row-bar" });
			const bar = barWrap.createDiv({ cls: `inkmath-telemetry-row-fill is-${entry.status}` });
			...
			const meta = row.createDiv({ cls: "inkmath-telemetry-row-meta", text });
			if (entry.coldStart) {
				meta.createSpan({ cls: "inkmath-telemetry-cold", text: "cold" });
			}
		}
	}
	...
	private updateCardCollapseUi(): void {
		if (this.logsCardEl) {
			...
			const button = this.logsCardEl.querySelector<HTMLButtonElement>(".inkmath-card-toggle");
			if (button) {
				setCompatibleIcon(
					button,
		...
		if (this.telemetryCardEl) {
			...
			const button = this.telemetryCardEl.querySelector<HTMLButtonElement>(".inkmath-card-toggle");
			if (button) {
				setCompatibleIcon(
					button,
...
class InkMathImageDebugModal extends Modal {
	...
	onOpen(): void {
		this.detachShell = attachInkMathModalEngine(this, { tone: "debug", size: "xl" });
		this.modalEl.addClass("inkmath-modal-shell");
		this.contentEl.addClass("inkmath-modal");
	...
	onClose(): void {
		...
		this.detachShell = null;
		this.modalEl.removeClass("inkmath-modal-shell");
		this.contentEl.removeClass("inkmath-modal");
		this.contentEl.empty();
	}
}
