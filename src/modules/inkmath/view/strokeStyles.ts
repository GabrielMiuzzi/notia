import type { InkMathStrokeStyle } from "../types";

type StrokeStyleOption = {
	value: InkMathStrokeStyle;
	label: string;
};

export const INKMATH_STROKE_STYLE_OPTIONS: StrokeStyleOption[] = [
...
const INKMATH_STROKE_STYLE_VALUES = new Set<string>(
	INKMATH_STROKE_STYLE_OPTIONS.map((option) => option.value)
);

const getDashUnitPattern = (style: InkMathStrokeStyle): number[] => {
	if (style === "dashed") {
		return [4, 2.4];
	}
	if (style === "dotted") {
...
const getPdfDashPattern = (style: InkMathStrokeStyle): number[] => {
	if (style === "dashed") {
		return [2, 1];
	}
	if (style === "dotted") {
...
export const isInkMathStrokeStyle = (value: unknown): value is InkMathStrokeStyle =>
	typeof value === "string" && INKMATH_STROKE_STYLE_VALUES.has(value);

export const resolveInkMathStrokeStyle = (value: unknown): InkMathStrokeStyle =>
	isInkMathStrokeStyle(value) ? value : "solid";
...
export const applyStrokeStyleToPdf = (
	pdf: { setLineDashPattern: (pattern: number[], phase: number) => void },
	style: InkMathStrokeStyle
): void => {
