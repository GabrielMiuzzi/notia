import type { InkMathPoint } from "../types";

const clamp01 = (value: number): number => Math.max(0, Math.min(1, value));

const clonePoint = (point: InkMathPoint): InkMathPoint => ({
	x: point.x,
	y: point.y,
...
export const smoothStrokePoints = (points: InkMathPoint[], smoothing: number): InkMathPoint[] => {
	if (points.length < 3) {
