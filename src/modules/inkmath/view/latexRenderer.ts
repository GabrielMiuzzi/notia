...
export const renderLatexSegments = async (
	...
	try {
		...
		const root = container.createDiv({ cls: "inkmath-latex-render-root" });
		...
		for (const segment of segments) {
			if (segment.type === "text") {
				const text = root.createDiv({ cls: "inkmath-markdown-render" });
			...
			const host = root.createDiv({
				cls: segment.displayMode
					? "inkmath-markdown-render inkmath-markdown-render--math"
					: "inkmath-markdown-render inkmath-markdown-render--math-inline"
			});
