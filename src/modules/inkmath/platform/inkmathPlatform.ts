export interface InkMathHostBridge {
  rootPath: string
...
const globalKey = '__notiaInkMathPlatformInitialized__'
...
function getNormalizedBridgeFilePaths(bridge: InkMathHostBridge): string[] {
...
class VaultAdapter {
  constructor(private readonly bridge: InkMathHostBridge) {}
...
class Vault {
  ...
  getAbstractFileByPath(path: string): TAbstractFile | null {
    ...
    // (e.g., on Android, display paths like "Library-name/file.inkmath" are
...
class MetadataCache {
  ...
  getFirstLinkpathDest(linkPath: string, sourcePath: string): TFile | null {
    ...
    if (!hasExtension) {
      for (const suffix of ['.md', '.inkmath']) {
        if (isAbsolutePath(normalizedTarget)) {
...
export class WorkspaceLeaf {
  ...
  constructor(private readonly appRef: App, private readonly bridge: InkMathHostBridge) {}
...
export class App {
  ...
  constructor(bridge: InkMathHostBridge) {
    installDomHelpers()
...
export class Menu {
  ...
  showAtPosition(position: { x: number; y: number }): void {
    ...
    root.className = 'menu inkmath-compat-menu'
  ...
  async render(_app: App, source: string, container: HTMLElement): Promise<void> {
    ...
    if (displayMathMatch) {
      try {
        ...
        const block = container.createDiv({ cls: 'inkmath-markdown-render inkmath-markdown-render--math' })
    ...
    const block = container.createDiv({ cls: 'inkmath-markdown-render' })
...
export function createInkMathApp(bridge: InkMathHostBridge): App {
